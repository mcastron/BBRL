...
void RNG::multinom(uint n, const std::vector<double>& probs, std::vector<uint>& samp)
{
...
void RNG::multinom(uint size, const double* probs, uint num_probs, uint* samp, uint &maxS)
{
  if (num_probs == 0) return;
  for (uint i = 0; i < num_probs; i++) samp[i] = 0;
  if (size == 0) return;
  ...
  samp[num_probs - 1] = size;
	if(samp[num_probs - 1] > maxsize){
		maxS = num_probs-1;
		//maxsize = samp[num_probs-1];
	}
}
void RNG::multinom(const double* probs, uint num_probs, uint &sample){
	double u = rand_open01();
	double sum = probs[0];
	for(uint i=0;i<num_probs;){
		if(u < sum){
			sample = i;
			return;
		}
