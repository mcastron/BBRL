...
#ifndef FIXEDFANN
void fann_compute_MSE_w(struct fann *ann, fann_type * desired_output, double weight);
...
float fann_train_epoch_batch_w(struct fann *ann, struct fann_train_data *data,
                               const double* weights)
{
	...
	{
		fann_run(ann, data->input[i]);
		fann_compute_MSE_w(ann, data->output[i], weights[i]);
		fann_backpropagate_MSE(ann);
		fann_update_slopes_batch(ann, ann->first_layer + 1, ann->last_layer - 1);
	}
...
float fann_train_epoch_incremental_w(struct fann *ann, struct fann_train_data *data,
                                     const double* weights)
{
	unsigned int i;
	...
	for(i = 0; i != data->num_data; i++)
	{
		fann_train_w(ann, data->input[i], data->output[i], weights[i]);
	}
                                                ...
                                                const double* weights)
{
	...
	{
	case FANN_TRAIN_BATCH:
		return fann_train_epoch_batch_w(ann, data, weights);
	case FANN_TRAIN_INCREMENTAL:
		return fann_train_epoch_incremental_w(ann, data, weights);
	default:
	     printf("\n\t\t\tUnsupported training algorithm (do not use weights !)\n");
	     return -1;
	}
	return 0;
}
                                                      ...
                                                      double* weights)
{
	...
	for(; dat < train_data->num_data; dat++)
	{
		...
		if(swap != dat)
		{
			...
			double tmp    = weights[dat];
			weights[dat]  = weights[swap];
			weights[swap] = tmp;
		}
	}
}
