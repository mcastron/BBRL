									   ...
									   double weight)
{
	...
	fann_compute_MSE_w(ann, desired_output, weight);
...
fann_type fann_update_MSE_w(struct fann *ann, struct fann_neuron* neuron, fann_type neuron_diff,
                            double weight)
{
	float neuron_diff2;
	...
	ann->MSE_value += weight*neuron_diff2;
...
void fann_compute_MSE_w(struct fann *ann, fann_type * desired_output, double weight)
{
	...
	for(; last_layer_begin != last_layer_end; last_layer_begin++)
	{
		...
		neuron_diff = fann_update_MSE_w(ann, last_layer_begin, neuron_diff, weight);
