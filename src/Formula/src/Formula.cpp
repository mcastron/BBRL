...
// ============================================================================
Formula::Formula(std::istream& is) : Serializable()
{
	try							{ dDeserialize(is);	}
...
void Formula::serialize(ostream& os) const
{
...
void Formula::deserialize(istream& is) throw (SerializableException)
{
	...
	if (!getline(is, tmp))    { throwEOFMsg("tokenList");              }
     try                       { tokenList = getTokensFromStr(tmp);     }
     catch (TokenException& e) { throw SerializableException(e.what()); }
	++i;
	
	
	//  'varSet'
	try                         { buildVarMap();                         }
	catch (FormulaException& e) { throw SerializableException(e.what()); }
